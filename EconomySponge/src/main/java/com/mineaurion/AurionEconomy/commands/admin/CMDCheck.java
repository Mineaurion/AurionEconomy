package com.mineaurion.AurionEconomy.commands.admin;
	...
	public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
		...
		if (m.matches()) {
			...
			if (transactionresult.getResult() == ResultType.SUCCESS) {
				...
				Main.sendmessage("Tu as retirer {{RED}}" + amountText.toPlainSingle() + "{{WHITE}} à {{YELLOW}}" + player.getName(),
						src.getName());
				...
				if (player.isOnline()) {
					Main.sendmessage("Tu as dépensé {{RED}}" + amountText.toPlainSingle() + "", player.getName());
				}
